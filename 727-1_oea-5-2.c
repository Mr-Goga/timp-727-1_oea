...
{
	list *arr = (list*)malloc(sizeof(list)), *tmp = (list*)malloc(sizeof(list));
	init1(arr);
	init1(tmp);
	node  *tmp_node = (node*)malloc(sizeof(node));
	tmp_node = t->root;
	push_back(arr, tmp_node->value);
	for (;;) 
	{
		if (tmp_node->right != NULL)
		{
			push_front(tmp, tmp_node->right->value);
		...
		else if (tmp_node->left == NULL)
		{
			...
			find(t, tmp->head->value, &tmp_node);
			push_back(arr, tmp->head->value);
